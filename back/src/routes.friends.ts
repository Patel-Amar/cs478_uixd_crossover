...
async function remove(req: Request, res: responseType) {
    try {
        ...
        if (friendID !== null || friendID !== undefined) {
            resp = await db.get(
                "SELECT id FROM friends WHERE friend1=? AND id=?",
                currentUserID,
                friendID
            );
            if (resp === undefined) {
                resp = await db.get(
                    "SELECT id FROM friends WHERE id=? AND friend2=?",
                    friendID,
                    currentUserID
                );
            }
        ...
        if (resp === undefined) {
            return res
                .status(400)
                .json({
                    error: "Friendship you are trying to remove does not exist or you do not have permissions to remove.",
                });
        }
        ...
        return res
            .status(500)
            .json({ error: "There was an error removing this friendship." });
    }
}
...
async function add(req: Request, res: responseType) {
    try {
        let { auth_token } = req.cookies;
        let resp = await db.get(
            "SELECT * FROM users users WHERE token = ?",
            auth_token
        );
        let currentUserID = resp.id;
        ...
        if (recFriendID !== null || recFriendID !== undefined) {
            if (recFriendID === currentUserID) {
                return res
                    .status(400)
                    .json({ error: "Can not send request to yourself" });
            }
            await db.run(
                "INSERT INTO friends(friend1, friend2, pending) VALUES(?,?,?)",
                currentUserID,
                recFriendID,
                1
            );
        ...
        if (err.code === "SQLITE_CONSTRAINT") {
            return res
                .status(400)
                .json({ error: "Friendship already exists." });
        }
        return res
            .status(500)
            .json({ error: "There was an error removing this friendship." });
...
async function viewFriends(req: Request, res: Response) {
    try {
        ...
        return res
            .status(500)
            .json({ error: "There was an error retrieving your friends." });
    }
}
...
async function viewRequests(req: Request, res: Response) {
    try {
        ...
        return res
            .status(500)
            .json({
                error: "There was an error retrieving your friend requests.",
            });
    }
}
...
async function accept(req: Request, res: Response) {
    try {
        ...
        if (friendID !== null || friendID !== undefined) {
            resp = await db.get(
                "SELECT id FROM friends WHERE friend2=? AND id=?",
                currentUserID,
                friendID
            );
        } else {
        ...
        if (resp === undefined) {
            return res
                .status(400)
                .json({
                    error: "Friendship you are trying to accept does not exist.",
                });
        }
        ...
        return res
            .status(500)
            .json({ error: "There was an error accepting this friendship." });
    }
}
    ...
    try {
        ...
        if (resp === undefined) {
            return res
                .status(400)
                .json({
                    error: "Friendship you are trying to decline does not exist.",
                });
        }
        ...
        return res
            .status(500)
            .json({ error: "There was an error declining this friendship." });
    }
}
...
async function searchUsers(req: Request, res: Response) {
    try {
        ...
        resp = await db.all(
            "SELECT u.id, u.username AS friend_username FROM users u \
                        WHERE u.id != ? AND u.username LIKE ? \
                        AND NOT EXISTS( \
                            SELECT 1 FROM friends f \
                            WHERE(f.friend1 = u.id AND f.friend2 = ?) \
                                OR(f.friend2 = u.id AND f.friend1 = ?))",
            currentUserID,
            `%${searchTerm}%`,
            currentUserID,
            currentUserID
